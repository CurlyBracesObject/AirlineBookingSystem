...
public class UserController {
    ...
    public ModelAndView login(@RequestParam("loginname") String loginname,
        ...
        boolean flag = false;
        if ("1".equals(tip)) {
            User user = rainservice.login(loginname, password, tip);
            if (user != null) {
                // 将用户保存到HttpSession当中
                ...
                // 设置登录失败提示信息
                System.out.println("Set login failure prompt information");
                mv.addObject("message", "Incorrect login name or password! Please re-enter");
                // 服务器内部跳转到登录页面
